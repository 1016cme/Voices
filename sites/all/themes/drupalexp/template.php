...
function template_preprocess_section(&$vars) {
  ...
  $vars['attributes_array']['class'] = $vars['classes_array'];
	if(isset($section->custom_id) && !empty($section->custom_id)){
		$vars['attributes_array']['id'] = drupal_html_id($section->custom_id);
	}else{
  	$vars['attributes_array']['id'] = drupal_html_id('section-' . $section->title);
	}
...
function drupalexp_preprocess_html(&$vars) {
  ...
  if (isset($library['font-awesome'])) {
    $path = $library['font-awesome'];
  ...
  $node = menu_get_object('node');
  if($node){
    $node_preset = variable_get($node->nid.'dexp_page_preset','not-set');
    $theme->preset = $node_preset == 'not-set'?$theme->preset:$node_preset;
    $theme->setPresetVars();
  }
...
/*************** add block to layer *******************/
function template_preprocess_dexp_layerslider_block_view($variables) {

  $view = $vars['view'];
  $vars['content'] = l($layer->$view, 'audio_samples', 'block_1');
