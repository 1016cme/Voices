  ...
  Drupal.behaviors.drupalexp_google_font = {
    attach: function (context, settings) {
      $('.google-font').once('google-font', function () {
        ...
        if ($this.hasClass('global-font')) {
          $style.attr('multiple', 'multiple');
          $style.parent().removeClass('col-sm-2').addClass('col-sm-4');
          $size.parent().hide();
          $height.parent().hide();
        }
        if (font[0] != '') {
          ...
          if (font_selected.length > 0) {
            ...
            $.each(font[1].split(','), function (i, e) {
              $style.find('option[value=' + e + ']').attr('selected', true);
            });
          }
        } else {
